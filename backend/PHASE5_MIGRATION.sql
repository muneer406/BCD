...
--
-- PII WARNING: The error_message column may contain sensitive information
-- such as stack traces, local file paths, image paths, or other operational
-- details. Treat this column as potentially containing PII and restrict access.
--
-- RETENTION POLICY: It is recommended to auto-delete rows older than 90 days
-- to limit storage growth and PII exposure.
--
-- CLEANUP: Configure a scheduled cleanup job (e.g., Supabase cron, pg_cron,
-- or an external worker) that runs at least daily to purge expired rows.
-- Example pg_cron job:
